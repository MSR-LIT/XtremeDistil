...
# -*- coding:utf-8 -*-
"""
# 2018 The Google AI Language Team Authors.
# 2019 The BioNLP-HZAU
...
flags.DEFINE_string(
    "data_dir", None,
    "The input data dir. Should contain train.tsv, dev.tsv, test.tsv files (or other data files) "
    "for the task.")
...
def main(_):
    ...
    if FLAGS.do_distil:
        ...
        x_train, y_train, x_teacher, distil_tokenizer, x_wt, x_wt_teacher, x_lang, y_lang, x_wt_lang = generate_sequence_data(FLAGS.output_dir, FLAGS.max_seq_length, FLAGS.data_dir+"/train.tsv", label_list, wordpiece_tokenizer, train=True, teacher_examples=predict_lines)

        dense_hidden_size = 768 #y_layer_teacher.shape[2]
        print ("Dimension of teacher's representation layer : ", str(dense_hidden_size))

        # x_dev, y_dev = None, None
        x_dev, y_dev, _, _, _, _, x_dev_lang, y_dev_lang, _, = generate_sequence_data(FLAGS.output_dir, FLAGS.max_seq_length, FLAGS.data_dir+"/dev.tsv", label_list, wordpiece_tokenizer, distil_tokenizer=distil_tokenizer)

        x_test, y_test, _, _, _, _, x_test_lang, y_test_lang, _, = generate_sequence_data(FLAGS.output_dir, FLAGS.max_seq_length, FLAGS.data_dir+"/test.tsv", label_list, wordpiece_tokenizer, distil_tokenizer=distil_tokenizer)
        ...
        word_emb, emb_size = get_word_embedding(FLAGS.word_embedding_file, wordpiece_tokenizer.vocab)
